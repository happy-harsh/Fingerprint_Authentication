    ...
    try:
        ...
        if (filename != ""):
            messagebox.showerror(
                "Incompatible File Error", "The file you have chosen is incompatible. Please select an image file and try again. If this error cannot be resolved, please report it to me at mauricerizat@gmail.com")
...
def removeBkg():
    ...
    try:
        ...
        # Create new image with same size as input image
        image2 = Image.new('RGBA', (width + 1, height))
        ...
        for i in range(image1.size[0]):  # Column
            for j in range(image1.size[1]):  # Row
                # Get individual red, green, blue and intensity values
                ...
                if (((r > lowerbound1 and r < upperbound1) and
                    ...
                    # If pixel is part of grid, make it transparent
                    pixels2[i, j] = (255, 255, 255, 0)
                else:
                    # Otherwise just copy the original pixel
                    pixels2[i, j] = pixels1[i, j]

        # Save new image to given output file in same directory
        image2.save(outputFile)
        ...
        messagebox.showerror(
            "Critical Error! 8990", "A critical error has occured. If this error cannot be resolved, please report it to me at mauricerizat@gmail.com")
...
# Header
appHeader = Label(window, text="Welcome to PNG Grid Background Remover",
                  bg="white", fg="black", font="none 26 bold italic")
#grid(row=0, column=0, sticky=W)

# Image to display

# Initial starter image. This cannot be processed
imageFirst = Image.open(resource_path("theme.jpg"))
# Resize image as it is only for reference purpose
...
selectFileButton = Button(inputStuff, text="Select File", font="none 15",
                          width=15, command=fileSelect)  # Button to select image file
toleranceText = Label(inputStuff, text="Select Tolerance Percentage: ",
                      fg="black", font="none 15")  # Tolerance text
toleranceSlider = Scale(inputStuff, from_=0, to=100, orient=HORIZONTAL,
                        width=15, length=200)  # Slider for tolerance
toleranceSlider.set(50)  # Default tolerance value is 50%
# Insert button in frame inputStuff
selectFileButton.grid(row=0, column=0, padx=30)
...
outputFileNameText = Label(inputStuff, text="Enter Output File Name (without extension): ",
                           fg="black", font="none 15")  # Accept output file name
# Text field for output file name
outputFileNameEntry = Entry(inputStuff, width=20, font="none 15")
# Default output file name is "defaultOutput"
outputFileNameEntry.insert(0, "defaultOutput")
# Insert output file name prompt in frams inputStuff
outputFileNameText.grid(row=1, column=0, padx=30, pady=10)
# Insert output file name text field in frame inputStuff
outputFileNameEntry.grid(row=1, column=1, pady=10)

# Remove Background button
removeBkgButton = Button(window, text="Remove Grid Background", font="none 15",
                         width=25, command=removeBkg)  # Button to begin background removal
...
bugsReport = Label(window, text="To report bugs and errors please reach me at mauricerizat@gmail.com",
                   bg="gray", fg="black", font="none 12 italic")
