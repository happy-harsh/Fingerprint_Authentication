    ...
    try:
        if len(path) > 0:
            ...
            for (x, y), value in np.ndenumerate(inv):
                if value == 0:
                    ...
                    if (inv[x - 1][y - 1] / 255) + (inv[x + 1][y - 1] / 255) + (inv[x][y - 2] / 255) + (
                        ...
                        print(
                            'RIDGE BIFURICATION OF MINUTIAE - COORDINATES : ', (x, y))
            ...
            harris_normalized = cv2.normalize(
                dst, 0, 255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32FC1)
...
def save_image():
    savePath = tkinter.filedialog.asksaveasfilename() + '.jpg'
...
downloadImageBtn = Button(
    root, text="Save result on computer", state=DISABLED, command=save_image)
downloadImageBtn.pack(side="top", fill="both",
