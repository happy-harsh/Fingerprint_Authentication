...
def finger():
    sample = cv.imread(
        'C:/Program Files/Mantra/MFS100/Driver/MFS100Test/FingerData/FingerImage.bmp', 0)
    ...
    if min(detect_SP['core'].shape) != 0:
        for i in range(0, detect_SP['core'].shape[0]):
            centre = (int(detect_SP['core'][i, 0]),
                      int(detect_SP['core'][i, 1]))
    ...
    if min(detect_SP['delta'].shape) != 0:
        for j in range(0, detect_SP['delta'].shape[0]):
            ...
            stacked_img = cv.polylines(
                stacked_img, [pts], True, (0, 255, 0), 2)
